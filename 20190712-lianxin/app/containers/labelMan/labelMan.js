...
class algorithmManagement extends Component {
  ...
  constructor(props) {
    ...
    this.columns = [
      {
        title: `${intl.get('lab-lab')}`,
        dataIndex: 'labelName',
        key: '0',
        width:"22%"
      },
      {
        title: `${intl.get('lab-pin')}`,
      ...
      for (let i = 0; i<result.data.data.data.length; i++) {
        if (sessionStorage.getItem("deleteAlgID")==result.data.data.data[i].id) {
          if(result.data.data.data[i].status==0){
             message.warning(intl.get('lab-list-modal-del-des'))
          }else {
            this.showModal()
          }
        }
  ...
  render() {
    return (
      <LocaleProvider  locale={zh_CN}>
        <div>
          <Breadcrumb>
            <Breadcrumb.Item className="crumb"><span>{intl.get('nav-dic-man')}</span><span className="sub cur"><span className="icon">/</span>{intl.get('nav-lab-man')}</span></Breadcrumb.Item>
          </Breadcrumb>
