...
class TaskQueue extends Component {
  constructor(props) {
    ...
    this.columns = [
      {
        title: intl.get('task-list-order'),
        key: '1',
        dataIndex: 'order',
        // width: "14%",
        ...
        render: (text, row, index) => {
          return (
            <div className="algBut">
              <a href="javascript:;" onClick={() => this.orderMoveUp(text, row, index)} className={this.setClassOrderMoveUp(row, index)}>{intl.get('btn-up')}</a>
              <a href="javascript:;" onClick={() => this.orderMoveDown(text, row, index)} className={this.setClassOrderMoveDown(row, index)}>{intl.get('btn-down')}</a>
              <a href="javascript:;" onClick={() => this.algDetails(text, row, index)} className="details">{intl.get('btn-vie')}</a>
  ...
  calcOrderMoveUp = (row, index) => {
    let { current, total } = this.state.pagination;
    if (current == 1 && index == 0) return true;
    return false;
  }
  setClassOrderMoveUp = (row, index) => {
    let not_allowed = '';
    if (this.calcOrderMoveUp(row, index)) {
      not_allowed = 'not_allowed'
    }
    return `details ${not_allowed}`;
  }
  calcOrderMoveDown = (row, index) => {
    let { current, total } = this.state.pagination;
    let totalPage = Math.ceil(total / this.state.param.param.pageRange);
    if (current == totalPage && index == this.state.data.length - 1) return true;
    return false;
  }
  setClassOrderMoveDown = (row, index) => {
    let not_allowed = '';
    if (this.calcOrderMoveDown(row, index)) {
      not_allowed = 'not_allowed'
    }
    return `details ${not_allowed}`;
  }
  ...
  orderMoveUp = (text, row, record) => {
    if (this.calcOrderMoveUp(row, record)) return;
    const { dispatch } = this.props
    orderMove.param = {
      ...orderMove.param,
      taskId: row.id
    }
    reduxSagaInjector(dispatch, 'ALG_LIST')('orderMoveUp', orderMove, (result) => {
      if (result.data.retCode === 0) {
        this.getDataList();
      } else {
        sage.error(result.data.errMsg);
      }
    })
  }
  orderMoveDown = (text, row, record) => {
    if (this.calcOrderMoveDown(row, record)) return;
    const { dispatch } = this.props
    orderMove.param = {
      ...orderMove.param,
      taskId: row.id
    }
    reduxSagaInjector(dispatch, 'ALG_LIST')('orderMoveDown', orderMove, (result) => {
      if (result.data.retCode === 0) {
        this.getDataList();
      } else {
        sage.error(result.data.errMsg);
...
export default withRouter(TaskQueue);
